...
static void treeFactory(GenericTree<int> &tree)
{
  ...
  if (a == nullptr)
  {
    a = new GenericTree<int>::TreeNode(9999);
  }
  else
  {
    a->data = 4;
    auto b = a->addChild(8);
    auto e = a->addChild(15);
    auto c = b->addChild(16);
    auto f = b->addChild(23);
    auto d = c->addChild(42);
  }
}
...
template <typename T>
std::vector<T> parentFind(typename GenericTree<T>::TreeNode *subtreeRoot)
{
  std::queue<typename GenericTree<T>::TreeNode *> nodesToExplore;
  std::vector<T> results;

  nodesToExplore.push(subtreeRoot);
  results.push_back(nodesToExplore.front()->data);
  ...
  while (!nodesToExplore.empty())
  {
    ...
    std::vector<typename GenericTree<T>::TreeNode *> compressedChildrenPtrs;
    ...
    topNode->childrenPtrs.swap(compressedChildrenPtrs);
  }
...
std::vector<T> traverseLevels(GenericTree<T> &tree)
{
  ...
  results = parentFind<T>(rootNodePtr);
  return results;
}
